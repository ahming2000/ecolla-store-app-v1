...
<div class="col-12">
    <div class="h6 text-muted">
        <span><?= $view->getTotalPurchaseCount($item); ?> sold</span> |
        <span><?= $item->getViewCount(); ?> views</span>
    </div>
</div>
        ...
        <?php for($i = 0; $i < sizeof($item->getWholesales()); $i++) : ?>
            <div class="h6 wholesale-view" <?= $i != 0 ? "hidden" : ""; ?>>
                ...
                <span class="grey-text">
                    Purchase until <?= $item->getWholesales()[$i]->getMin() ?> pcs will get wholesale price RM<?= number_format($wholesalePrice, 2, '.', ''); ?>
                </span>
            </div>
        <?php endfor; ?>
