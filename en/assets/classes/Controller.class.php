...
class Controller extends Model {
    ...
    public function insertNewItem($itemName){
        ...
        // Check duplication
        if($this->dbSelectRow("items", "i_name_en", $itemName) != null) return false;
    ...
    public function updateItem($oldItem, $newItem, $i_id){
        ...
        if($oldItem == $newItem) return true;
            ...
            /* Item */
            // Item name
            if ($oldItem->getName() != $newItem->getName()) $this->dbUpdate("items", "i_name_en", $newItem->getName(), "i_id", $i_id);

            // Item description
            if ($oldItem->getDescription() != $newItem->getDescription()) $this->dbUpdate("items", "i_desc", $newItem->getDescription(), "i_id", $i_id);

            // Item brand
            if ($oldItem->getBrand() != $newItem->getBrand()) $this->dbUpdate("items", "i_brand_en", $newItem->getBrand(), "i_id", $i_id);

            // Item Origin
            if ($oldItem->getOrigin() != $newItem->getOrigin()) $this->dbUpdate("items", "i_origin_en", $newItem->getOrigin(), "i_id", $i_id);

            // Item property name
            if ($oldItem->getPropertyName() != $newItem->getPropertyName()) $this->dbUpdate("items", "i_property_name_en", $newItem->getPropertyName(), "i_id", $i_id);
            ...
            foreach($categoryToRemove as $cat){
                $cat_id = $this->dbSelectAttribute("categories", "cat_id", "cat_name_en", $cat);
            ...
            foreach($barcodeRemained as $b){
                ...
                // Variety property
                if($oldV->getProperty() != $newV->getProperty()) $this->dbUpdate("varieties", "v_property_en", $newV->getProperty(), "v_barcode", $b);
    ...
    public function addViewCount($item){
        $this->dbUpdate("items", "i_view_count", $item->getViewCount() + 1, "i_name_en", $item->getName());
    }
    ...
    public function list($itemName){
        ...
        $i_id = $this->dbSelectAttribute("items", "i_id", "i_name_en", $itemName);
    ...
    public function changeListStatus($itemName){
        if($this->dbSelectAttribute("items", "i_is_listed", "i_name_en", $itemName) == 1){
            $this->dbUpdate("items", "i_is_listed", 0, "i_name_en", $itemName);
            return true;
        } else{
